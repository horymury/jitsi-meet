...
import { getCssOverride } from '../../../react/features/css-override/functions';
...
export default class SmallVideo {
    ...
    focus(isFocused) {
        ...
        const focusOverride = getCssOverride('Thumbnails')?.focused || {};

        if (!isFocused && isFocusClassEnabled) {
            this.$container.removeClass(focusedCssClass);
            Object.keys(focusOverride).forEach(prop => {
                this.$container[0].style[prop] = '';
            });
        } else if (isFocused && !isFocusClassEnabled) {
            this.$container.addClass(focusedCssClass);
            Object.keys(focusOverride).forEach(prop => {
                this.$container[0].style[prop] = focusOverride[prop];
            });
        }
    }
    ...
    showDominantSpeakerIndicator(show) {
        ...
        const activeSpeakerOverride = getCssOverride('Thumbnails')?.activeSpeaker || {};

        if (show) {
            Object.keys(activeSpeakerOverride).forEach(prop => {
                this.$container[0].style[prop] = activeSpeakerOverride[prop];
            });
        } else {
            Object.keys(activeSpeakerOverride).forEach(prop => {
                this.$container[0].style[prop] = '';
